...
public class ChessModel implements ChessGameModel {
	...
	private Couleur currentPlayer, unCurrentPlayer;
	...
	public ChessModel() {
		...
		this.unCurrentPlayer = (Couleur.BLANC.equals(this.currentPlayer) ? Couleur.NOIR : Couleur.BLANC);
	...
	/* (non-Javadoc)
	 * @see model.business.ChessGameModel#getPieceListMoveOK(int, int)
	 * 
	...
	private void switchJoueur() {
		Couleur temp = this.currentPlayer;
	...
	private ActionType isMoveOK(ModelCoord initCoord, ModelCoord finalCoord) {
		...
		if ( ! ChessModelConfig.coordonnees_valides(initCoord) ||
				! ChessModelConfig.coordonnees_valides(finalCoord)) {
